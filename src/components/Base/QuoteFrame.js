...
class QuoteFrame extends React.Component {
	render() {
		...
		let fullSizeHeader = lastRoute.fullSizeHeader;

		const route = this.props.routes.find(route => route.storeref);

		if (route) {
			let { storeref } = route;
			storeref = storeref.getState().getIn(['data','quote']).toJS();
			if (!storeref.researchObjective) { fullSizeHeader = false };
		}
